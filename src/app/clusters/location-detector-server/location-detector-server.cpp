...
void MatterLocationDetectorPluginServerInitCallback()
{
    ReturnOnFailure(CommandHandlerInterfaceRegistry::Instance().RegisterCommandHandler(&LocationDetectorServer::Instance()));
    VerifyOrReturn(AttributeAccessInterfaceRegistry::Instance().Register(&LocationDetectorServer::Instance()),
                   CHIP_ERROR_INCORRECT_STATE);
}

void emberAfLocationDetectorClusterServerInitCallback(chip::EndpointId endpoint)
{
    ChipLogProgress(Zcl, "Creating LocationDetector cluster, Ep %d", endpoint);
    LocationDetectorServer::Instance().RegisterEndpoint(endpoint);
}

void MatterLocationDetectorClusterServerShutdownCallback(chip::EndpointId endpoint)
{
    ...
    LocationDetectorServer::Instance().UnregisterEndpoint(endpoint);
}
// -----------------------------------------------------------------------------
...
LocationDetectorContent::LocationDetectorContent() : LocationDetectorContent(kInvalidEndpointId) {}

LocationDetectorContent::LocationDetectorContent(EndpointId aEndpoint)
{
    endpoint = aEndpoint;
...
CHIP_ERROR LocationDetectorServer::Read(const ConcreteReadAttributePath & aPath, AttributeValueEncoder & aEncoder)
{
...
CHIP_ERROR LocationDetectorServer::Write(const ConcreteDataAttributePath & aPath, AttributeValueDecoder & aDecoder)
{
    CHIP_ERROR err     = CHIP_NO_ERROR;
    auto endpoint      = aPath.mEndpointId;
    auto endpointIndex = EndpointIndex(endpoint);
...
LocationDetectorServer & LocationDetectorServer::Instance()
{
    static LocationDetectorServer LocationDetectorServer;
    return LocationDetectorServer;
}

void LocationDetectorServer::Shutdown()
{
...
size_t LocationDetectorServer::GetNumSupportedEndpoints() const
{
    return kNumSupportedEndpoints;
}

CHIP_ERROR LocationDetectorServer::RegisterEndpoint(EndpointId endpointId)
{
    ...
    content[endpointIndex] = LocationDetectorContent(endpointId);
    return CHIP_NO_ERROR;
}

CHIP_ERROR LocationDetectorServer::UnregisterEndpoint(EndpointId endpointId)
{
...
size_t LocationDetectorServer::EndpointIndex(EndpointId endpointId) const
{
    for (size_t i = 0; i < kNumSupportedEndpoints; ++i)
    {
...
size_t LocationDetectorServer::NextEmptyIndex() const
{
    for (size_t i = 0; i < kNumSupportedEndpoints; ++i)
    {
